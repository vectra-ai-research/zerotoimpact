...
export const ATTACK_LIST: Attack[] = [
  ...
  {
    id: 2,
    name: "IAM Policy Rollback to Suspicious S3 Encryption",
    description:
      "In this scenario, an attacker with initial, limited IAM access exploits SetDefaultPolicyVersion to restore an older policy version with full administrative rights. The attacker then creates a new S3 administrator and encrypts objects in an S3 bucket using a client-provided encryption key. The simulation replaces one existing object with an encrypted PutObject request and encrypts the remaining objects in place using CopyObject. This models ransomware-style loss of access without relying on KMS.",
    platform: "AWS",
  ...
  2: [
    ...
    },
    {
      ...
      description: {
        ...
        content:
          "Creating an S3 bucket filled with files that the attacker will encrypt with a client-provided key as part of a ransomware simulation.",
      },
    }
  ],
  3: [
  ...
  2: [
    ...
    },
    {
      ...
      description: {
        ...
        content:
          "To retain access after the policy rollback, the attacker creates a new IAM user with broad S3 permissions. The user is then used to encrypt objects with a client-provided key.",
      }
    },
    {
      title: "Impact",
      ...
      description: {
        ...
        content:
          "The attacker replaces one existing S3 object with an encrypted version using PutObject, then encrypts the remaining objects in place using CopyObject. Because the client-provided key is not retained, the objects cannot be read through the simulation without that key, disrupting data availability.",
      }
    },
  ],
  3: [
