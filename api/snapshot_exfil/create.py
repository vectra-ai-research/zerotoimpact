...
AMAZON_LINUX_2023_AMI_PARAMETER = '/aws/service/ami-amazon-linux-latest/al2023-ami-kernel-default-x86_64'
...
class Create:
    ...
    def create(self):
        ...
        try:
            ...
            self.step = 2
            self.launch_ec2_instance(self.get_latest_amazon_linux_ami(), "t2.micro", role_name)
            self._add_to_disk()
    ...
    def _add_to_disk(self):
        add_to_disk(self.filename, self.id, self.status, self.step, self.api_logs, self.important_logs, self.resources, region=self.region)
    ...
    def get_latest_amazon_linux_ami(self):
        ...
        response = ssm_client.get_parameter(Name=AMAZON_LINUX_2023_AMI_PARAMETER)
        image_id = response['Parameter']['Value']
        self.log_api_call('get Amazon Linux 2023 AMI parameter', response)
        self.log_important(f"Resolved the Amazon Linux 2023 AMI for {self.region}.")
        return image_id
    ...
    def create_rds_instance(self, db_instance_identifier, db_instance_class, db_engine, master_username, master_user_password):
        ...
        # Persist the database identifier before waiting. If the process is
        # interrupted while RDS provisions, teardown can still find and remove
