...
class CreateLambdaPriEsc:
    def __init__(self, id, profile, aws_region, pathToDisk):
        self.id = id
        self.aws_region = aws_region
        self.profile = profile 
        self.client = create_client_profile('iam', self.aws_region, self.profile)
        self.logs =  []
        self.exchange = []
        self.resources = []
        self.resources_v2 = {
    ...
    def _add_to_disk(self):
        add_to_disk(self.filename, self.id, self.status, self.step, self.exchange, self.logs, self.resources, self.resources_v2, self.aws_region)
