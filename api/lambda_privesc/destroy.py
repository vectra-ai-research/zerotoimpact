...
class DestroyLambdaPriEsc:
    ...
    def destroy(self):
        ...
        try:
            # A Lambda function keeps its execution role in use, so remove the
            # function before deleting the debug role it assumes.
    ...
    def _add_to_disk(self):
        add_to_disk(self.filename, self.id,self.status,self.step,self.instance["exchange"],self.instance["logs"],self.instance["resources"], self.instance["resourcesV2"], self.aws_region)
