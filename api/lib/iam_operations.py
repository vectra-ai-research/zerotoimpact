...
def console_login(user_name, access_key_id, secret_access_key, exchange, logs):
    ...
    # STS federation token names are limited to 32 characters. Scenario IDs
    # make the IAM username longer than that, so retain a valid prefix.
    federation_name = user_name[:32]
    ...
    logs.append("Console sign-in URL created.")
...
def get_profile_account_id(profile, region):
